...
const SECTIONS = [
  ...
  { id: 'afternoon-2', label: 'Sera', hours: [19, 20, 21, 22, 23, 0, 1, 2, 3, 4, 5] },
];
...
export default function App() {
  ...
  const [newTask, setNewTask] = useState({ title: '', note: '', priority: false });
  ...
  useEffect(() => {
    if (focusedTaskIndex !== -1) {
      ...
      const filteredTasks = tasks.filter(t => t.sectionId === currentSectionId && (showCompleted || !t.completed));
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      const visibleSectionTasks = tasks.filter(t => t.sectionId === currentSectionId && (showCompleted || !t.completed));
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 'j':
          if (visibleSectionTasks.length > 0) {
            setFocusedTaskIndex(prev => (prev < visibleSectionTasks.length - 1 ? prev + 1 : 0));
          }
          break;
        case 'k':
          if (visibleSectionTasks.length > 0) {
            setFocusedTaskIndex(prev => (prev > 0 ? prev - 1 : visibleSectionTasks.length - 1));
          }
          break;
        case 'x':
          if (focusedTaskIndex !== -1 && visibleSectionTasks[focusedTaskIndex]) {
  ...
  }, [isModalOpen, activeQuarterIndex, focusedTaskIndex, tasks, newTask, showCompleted]);
    ...
    */}
      ...
      <header className="p-6 flex justify-between items-center max-w-7xl mx-auto">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">Organizza la giornata del gambero</p>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 h-[calc(100vh-160px)]">
        {SECTIONS.map((section, idx) => {
          ...
          const sectionTasks = tasks.filter(t => t.sectionId === section.id && (showCompleted || !t.completed));
          ...
          return (
            ...
            >
              {/* Section Header */}
              <div className={`p-6 flex items-center justify-between ${isCurrentTime ? 'bg-orange-50/50' : ''}`}>
                <div>
                  <h2 className={`text-xl font-bold ${isFocused ? 'text-orange-600' : 'text-slate-700'}`}>
                    {section.label}
                  </h2>
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1">
                    <Clock size={12} /> {section.hours[0]}:00 - {section.hours[section.hours.length-1]}:59
                  </span>
                </div>
                {isCurrentTime && (
                  ...
                  sectionTasks.map((task, tIdx) => (
                    ...
                    >
                      <div className="flex gap-3">
                        <div className="mt-1">
                          {task.completed ? (
                            ...
                            <div className={`w-[18px] h-[18px] rounded-full border-2 ${task.priority ? 'border-orange-500 bg-orange-50' : 'border-slate-200'}`} />
                          )}
                        </div>
                        <div className="flex-1">
                          <h3 className={`font-bold leading-tight ${task.completed ? 'line-through text-slate-400' : 'text-slate-800'}`}>
                            {task.title}
                          </h3>
                          {task.note && (
                            <p className="text-xs text-slate-500 mt-1 line-clamp-2">{task.note}</p>
                          )}
                        </div>
                      ...
                      <div className="absolute right-2 top-2 hidden group-hover:flex gap-1">
                        <button onClick={(e) => { e.stopPropagation(); deleteTask(task.id); }} className="p-1 hover:text-red-500 transition-colors">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
      ...
      <footer className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-6 py-3 rounded-full shadow-2xl flex gap-6 text-xs font-medium z-40 border border-slate-700 whitespace-nowrap">
        <div className="flex gap-2 items-center"><span className="bg-slate-700 px-1.5 py-0.5 rounded text-[10px] font-bold">SPACE</span> Crea 🦐</div>
        <div className="flex gap-2 items-center"><span className="bg-slate-700 px-1.5 py-0.5 rounded text-[10px] font-bold text-orange-400">H/L</span> Quarti</div>
        <div className="flex gap-2 items-center"><span className="bg-slate-700 px-1.5 py-0.5 rounded text-[10px] font-bold">J/K</span> Task</div>
        <div className="flex gap-2 items-center"><span className="bg-slate-700 px-1.5 py-0.5 rounded text-[10px] font-bold text-red-400">X</span> Elimina</div>
      </footer>
      ...
      <Dialog isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <div className="p-8 relative">
          ...
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-black text-slate-800 tracking-tight">Cosa bolle in pentola?</h2>
          </div>
          <p className="text-xs font-bold text-slate-400 mb-4 uppercase">Aggiunta a: <span className="text-orange-500">{SECTIONS[activeQuarterIndex].label}</span></p>
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-2">
               <div className="flex items-center gap-2">
                 <button 
                  onClick={() => setNewTask(prev => ({ ...prev, priority: !prev.priority }))}
               ...
               <span className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">Premi INVIO per salvare</span>
            </div>
            <button 
              onClick={createTask}
